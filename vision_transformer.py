...
# from prompt_fc import EPrompt
# from prompt_para import EPrompt

# from attention import PreT_Attention
from attention_ import PreT_Attention
# from attention_2 import PreT_Attention
...
class MyAttention(nn.Module):
    ...
    def forward(self, x, prompts, return_all_tokens=False):
        ...
        if return_all_tokens:
            ...
            x_cls = (attn @ v).transpose(1, 2).reshape(B, self.more_prompts*N, C)
...
class Block(nn.Module):
    ...
    def forward(self, x, down_key=None, down_value=None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), down_key, down_value)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            ...
            # self.MLPs_in = MlpDY(in_features=embedding_dim, out_features=embedding_dim, act_layer=nn.GELU)
            
            self.MLPs_in = nn.ModuleDict(OrderedDict({f'MLPs_in{i}': MlpDY(in_features=embedding_dim, out_features=embedding_dim, act_layer=nn.GELU) for i in range(11)}))
            
            # self.prompt_pool = nn.ModuleList([MlpDY(in_features=embedding_dim, out_features=embedding_dim, act_layer=nn.GELU) for i in range(11)])
            
            self.cross_attn_wo_x = True
            self.attnDY = nn.ModuleDict(OrderedDict({f'attn{i}': MyAttention(embedding_dim,
                                                                           num_heads=12,
                                                                           qkv_bias=True,
                                                                           attn_drop=0.2,
                                                                           proj_drop=0.2,
                                                                           ...
                                                                           residual=True) for i in
                                                   range(11)}))
            self.norms = nn.ModuleDict(
                OrderedDict({f'norm{i}': nn.LayerNorm(embedding_dim) for i in range(11)}))
            ...
            self.dap_downsample = nn.ModuleList([
                    nn.Linear(197, 20)
                    for i in range(11)              #################   8
                ])
            for i in range (11):              #################   8
                ...
                # nn.init.uniform_(self.dap_downsample[i].weight, -1, 1)   ### .zeros_(self.dap_downsample[i].weight)   .uniform_(self.dap_downsample[i].weight, -1, 1)
                # nn.init.uniform_(self.dap_downsample[i].bias, -1, 1)        ### .zeros_(self.dap_downsample[i].bias)   .uniform_(self.dap_downsample[i].bias, -1, 1)
            self.dap_norm = nn.LayerNorm(768, eps=1e-6)
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        ...
                        # res = self.e_prompt(x, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=cls_features[:, 0])
                        ...
                        if self.use_prefix_tune_for_e_prompt:
                            ...
                            if e_prompt_counter > 0:
                                ...
                                ne_prompt = e_prompt
                            #############
                            dap_downsample_ = self.dap_downsample[e_prompt_counter]
                            
                            ###############
                            cls_features_n = self.norm3(cls_features)
                            tokens = self.MLPs_in[f'MLPs_in{e_prompt_counter}'](cls_features_n)
                            
                            q_new = x
                            q_new, attn_new = self.attnDY[f'attn{e_prompt_counter}'](q_new, tokens )
                            ...
                            # ), dim=1)
                            #############
                            x_norm = self.dap_norm(q_new)
                            # x_norm = self.dap_norm(x)
                            x_tran = torch.transpose(x_norm, 2, 1)
                            down = dap_downsample_(x_tran)

                            ne_prompt = ne_prompt.permute(1, 0, 2, 3, 4).contiguous() # 2, B, prompt_length, num_heads, C // num_heads
                            _, B, prompt_length, num_heads, C_num_heads = ne_prompt.shape

                            key_prefix = ne_prompt[0].reshape( B, -1 ) # B,  prompt_length, num_heads,embed_dim // num_heads
                            value_prefix = ne_prompt[1].reshape( B, -1 ) # B,  prompt_length, num_heads, embed_dim // num_heads
                            ...
                            down_key = gamma4 * down + beta4
                            down_key = torch.transpose(down_key, 2, 1)
                            down_key = down_key.reshape( B, 20, num_heads, C_num_heads ).permute(0, 2, 1, 3).contiguous()       ####################
                            ...
                            gamma4 = gamma4.div(gamma_norm).view(film.size(0), -1, 1)
                            beta4 = beta4.div(beta_norm).view(film.size(0), -1, 1)
                            down_value = gamma4 * down + beta4
                            down_value = torch.transpose(down_value, 2, 1)          #######  B 20 768
                            down_value = down_value.reshape( B, 20, num_heads, C_num_heads ).permute(0, 2, 1, 3).contiguous()           ##########
                            ...
                            x = block(x, down_key=down_key, down_value=down_value)
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    pretrained_cfg['file'] = '../convp/B_16-i21k.npz'
