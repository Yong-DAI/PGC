...
def split_single_dataset(dataset_train, dataset_val, args, dataset_feat_train=None,):
    ...
    if args.dataset.replace('Split-','') == 'CIFAR100':
        # random.shuffle(labels)
        labels = [21, 87, 69, 91, 58, 17, 76, 4, 66, 0, 51, 96, 39, 14, 8, 15, 92, 10, 29, 28, 79, 18, 80, 53, 75, 27, 95, 40, 12, 60, 83, 73, 93, 46, 33, 13, 71, 43, 42, 11, 47, 78, 89, 59, 36, 23, 31, 26, 94, 74, 32, 57, 64, 99, 56, 35, 86, 16, 9, 44, 41, 1, 61, 62, 77, 97, 25, 72, 81, 38, 20, 30, 54, 6, 55, 52, 7, 90, 48, 5, 67, 63, 85, 3, 2, 68, 88, 22, 98, 37, 19, 50, 82, 49, 45, 24, 65, 84, 34, 70]
    # elif args.dataset.replace('Split-','') == 'Imagenet-R':
    #     random.shuffle(labels)   ### 77.26
    #     random.shuffle(labels)   #### 77.3
    #     random.shuffle(labels)  ###  77.6167
        # random.shuffle(labels)    ###  77.78
    elif args.dataset.replace('Split-','') == 'CUB200':
        random.shuffle(labels)   ### 75.99
